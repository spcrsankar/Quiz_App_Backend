...
exports.attemptQuiz = async (quizId, userId, answers, res) => {
  console.log("attempt quiz called===============")
  try {
    ...
    if(user.scores.length>0){
      ...
      participated = user.scores.some(
        (score) => quiz._id.toString() === score.Quiz_id.toString()
      );
    ...
    if (!participated) {
      ...
      await quiz.save();
      return res.status(201).json({ message: "Quiz attempted successfully" });
    }
    return res.status(201).json({ message: "Quiz already attempted" });
...
exports.displayQuestions = async (req, res) => {
  try {
    ...
    if (q.Creator_id && q.Creator_id.toString() === userId.toString()) {
      return res
        .status(206)
        .json({ message: "The creator of the quiz cannot attempt the quiz." });
    }
